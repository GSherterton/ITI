// Código de um compressor de Huffman com modelagem não contextual e adaptativo de decremento
...
// Variáveis globais
vector<char> alfabeto = {'a', 'b', 'c', 'd', 'r'};
string mensagem_original = "abracadabra";
...
void codificate_message(){
  ...
  for(int i = 0; i < mensagem_original.size(); i++){
    ...
    generate_code(); // gera os códigos de Huffman para cada símbolo
    mensagem_codificada += tabela_de_codigos[mensagem_original[i]]; // codifica o simbolo atual
    cont[mensagem_original[i]]--; // decrementa a contagem do símbolo que acabei de codificar
  }
...
void decodificate_message(){
  ...
  for(int i = 0; i < mensagem_codificada.size(); i++){
    ...
    if((aux_node->left == nullptr) && (aux_node->right == nullptr)){ // chegou em uma folha
      ...
      // reseta a árvore com a nova contagem
      cont[aux_node->simbolo]--; // decrementa a contagem do símbolo que acabei de codificar
      init_nos(); // inicializa os nós inicialmente
      root = build_huffman_tree(); // constrói a árvore de Huffman
